...
class TestDoctorCheckInGuard:
    ...
    def setup_data(self, db):
        ...
        pq = PatientQueue.objects.create(
            queue=queue, patient=patient, position=1, status="WAITING"
        )
    ...
    def test_call_next_succeeds_after_doctor_checkin(self, client, setup_data):
        ...
        # Manually set appointment to CHECKED_IN and queue check in time to simulate doctor check-in
    ...
    def test_doctor_qr_checkin_updates_appointments(self, client, setup_data):
        ...
        qr_data = f"QUEUE-TOKEN-{setup_data['doctor'].pk}-{timezone.localtime().strftime('%Y%m%d')}"
