...
class Patient(models.Model):
    ...
    user = models.OneToOneField(settings.AUTH_USER_MODEL, on_delete=models.CASCADE,
                                primary_key=True, related_name='patient_profile')
    ...
    emergency_contact = models.CharField(
        validators=[emergency_regex], max_length=10, blank=True)
...
class PatientForm(models.Model):
    ...
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name='medical_forms')
    symptoms = models.TextField(
        blank=True, help_text="List of selected symptoms")
    chief_complaint = models.TextField(help_text="Main reason for visit")
    medical_history_options = models.TextField(
    ...
    allergy_options = models.TextField(
