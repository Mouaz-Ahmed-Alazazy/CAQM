...
class TestDoctorModel:
    ...
    def test_get_available_slots_for_date_with_availability(self, doctor):
        ...
        # Should have 16 slots (9:00-17:00 in 30-min intervals)
        # 9:00, 9:30, 10:00, ..., 16:30 (last slot that fits before 17:00)
        assert len(slots) == 16
    ...
    def test_get_available_slots_respects_max_15_appointments(self, doctor, db):
        ...
        for i in range(15):
            ...
            patient_i = Patient.objects.create(user=user, address=f"{i} Test St")
    ...
    def test_get_available_slots_excludes_checked_in_appointments(
        self, doctor, patient
    ):
